...
.spill{font-family:var(--mono);font-size:11px;font-weight:700;padding:4px 11px;border-radius:999px;border:1px solid}
.s-ok{color:var(--teal);border-color:#1c5e44;background:#0a1f17}
.s-fail{color:var(--org);border-color:#5e3318;background:#1f120a}
.s-none{color:var(--mut);border-color:var(--line);background:#0d1a27}
.sbars{display:flex;gap:8px;align-items:flex-end;height:84px;margin-top:14px}
.sbar{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%;gap:5px}
.sbar-fill{width:72%;background:linear-gradient(180deg,#ffb703,#fb5607);border-radius:3px 3px 0 0;min-height:2px;transition:height .3s}
.sbar-lab{font-family:var(--mono);font-size:9px;color:var(--mut)}
...
<div class="card">
  <div class="srow"><span class="lbl" style="margin:0">&Scaron;&#770; · SECONDARY PATH</span><span class="spill s-none" id="sStatus">not calibrated</span></div>
...
function apply(d){
  ...
  var mp=document.getElementById('mode'); mp.textContent=d.mode||'idle';
  mp.className='pill '+(d.mode==='S-FAIL'?'lock-off':d.mode==='running'?'lock-on':'mode');
...
function renderS(d){
  var s=d.smag||[], st=document.getElementById('sStatus');
  if(d.mode==='S-FAIL'){st.textContent='FAIL — probe not heard';st.className='spill s-fail';}
  else if(d.cal){st.textContent='calibrated ✓';st.className='spill s-ok';}
  else{st.textContent='not calibrated';st.className='spill s-none';}
  var mx=Math.max(0.001,...s), h='';
  for(var i=0;i<ND;i++){var pct=Math.round(100*(s[i]||0)/mx);
    h+='<div class=sbar><div class=sbar-fill style=height:'+pct+'%></div><span class=sbar-lab>o'+(i+1)+'</span></div>';}
